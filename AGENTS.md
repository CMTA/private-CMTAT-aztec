...
- **Batching caps.** Two globals, both **measured, not derived**. `MAX_ADDR_PER_CALL = 4` for `mint_batch` and `burn_batch` (5 breaks them). `MAX_TRANSFER_ADDR_PER_CALL = 2` for `transfer_batch` (3 and above abort with `push out of bounds`), lower because each recipient costs four constrained deliveries once the `Transfer` event is delivered constrained to both the recipient and the issuer. The nested-private-call limit is irrelevant: the `_*_internal` helpers are `#[internal]` and inlined, so a batch makes no nested calls. Raising this global means re-running the suite at the new value and updating every batch test's array literals, not re-reading the protocol constants; see *Batching limits* in the README.
