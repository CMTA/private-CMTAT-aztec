...
- **Three deployment variants, one library.** `CMTATAztec` (base), `CMTATAztecDebt` (adds credit events and debt) and `CMTATAztecLight` (drops the validation module) are separate packages that compose modules from `cmtat_aztec_lib`. Two more packages, `CMTATAztecAuth` and `CMTATAztecAuthMultiToken`, are **not tokens**: they are ARC-403 authorization contracts built from the same modules that apply pause, deactivation and freeze to the `aztec-standards` AIP-20 `Token` / ARC-1155 `MultiToken` through the hook those tokens call before every transfer and burn (`doc/auth/README.md`). Two contracts because the hook signatures differ (`id` on the MultiToken) and Noir has no overloading; the rules live once in `lib/src/modules/authorizationHookModule.nr`, keyed on the burn selectors of the reference contracts, and the private hook checks the freeze flag in private and enqueues one public call carrying only `is_burn` for the pause/deactivation check (the H-3 decision applied). AIP-721 has no hook in the fork, so it is not covered. Adding an entry point to a shared module means adding it to **every variant that should expose it** — there is no inheritance to do it for you. Keep the three `main.nr` files in step.
...
├── cmtat-aztec-auth/                # CMTATAztecAuth — ARC-403 hook (pause, deactivation, freeze) for the AIP-20 Token
│   └── src/{main.nr, test.nr, test/{utils,test_hook,test_admin}.nr}
