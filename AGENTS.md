...
│   └── src/{main.nr, test.nr, test/{utils,test_hook,test_lists,test_admin}.nr}
├── cmtat-aztec-aip20/               # CMTATAztecAIP20 — aztec-standards Token @5433e9c verbatim + CMTAT terms/tokenId/roles/version
│   └── src/{main.nr, test.nr, test/{utils,test_aip20,test_cmtat}.nr}
└── arc403-interface/                # AuthorizationContract — signature-only stub of CMTATAztecAuth; not a member, never deployed
    └── src/main.nr
...
- Each contract's `main.nr` holds `VERSION`, a compile-time `str<31>` returned by `version()`. It is not stored state, so it cannot drift from the code — but it must be bumped by hand with every release, in all **six** contracts (the two authorization contracts and `CMTATAztecAIP20` deliberately carry the token contracts' version); the `CHANGELOG.md` checklist carries the step.
