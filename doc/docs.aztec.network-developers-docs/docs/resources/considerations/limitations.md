...
## Limitations developers need to know about[​](#limitations-developers-need-to-know-about)

- The Aztec stack is unaudited and under active development. See the [Alpha Network](https://docs.aztec.network/participate/alpha) page for details on what this means.
...
- Not all Noir cryptographic primitives work in public (AVM) functions. Signature verification (ECDSA secp256k1/r1), AES-128, Blake2s, and Blake3 are not supported. See [AVM Cryptographic Compatibility](https://docs.aztec.network/developers/docs/foundational-topics/advanced/circuits/avm_compatibility) for details and workarounds.
...
### It is not audited[​](#it-is-not-audited)
...
### Keys and addresses may change in future rollup versions[​](#keys-and-addresses-may-change-in-future-rollup-versions)
...
### No privacy-preserving queries to nodes[​](#no-privacy-preserving-queries-to-nodes)
...
### Limited private data authentication[​](#limited-private-data-authentication)
...
### No client-side bytecode validation[​](#no-client-side-bytecode-validation)
...
#### What are the consequences?[​](#what-are-the-consequences-1)
...
### New privacy standards are required[​](#new-privacy-standards-are-required)

There are many [patterns](https://docs.aztec.network/developers/docs/resources/considerations/privacy_considerations) which can leak privacy, even on Aztec. Standards have not been developed yet to encourage best practices when designing private smart contracts.

#### What are the consequences?[​](#what-are-the-consequences-3)

For example, until community standards are developed to reduce the uniqueness of ["Tx Fingerprints"](https://docs.aztec.network/developers/docs/resources/considerations/privacy_considerations#function-fingerprints-and-tx-fingerprints), app developers might accidentally forfeit some function privacy.
...
## Circuit limitations[​](#circuit-limitations)

### Upper limits on function outputs and transaction outputs[​](#upper-limits-on-function-outputs-and-transaction-outputs)
...
**In particular, these *per-transaction* limits will limit transaction call stack depths**. This means if a function call results in a cascade of nested function calls, and each of those function calls outputs many state reads and writes, or logs, then all of that accumulated output data might exceed the per-transaction limits that we currently have. This would cause such transactions to fail.
...
[Edit this page](https://github.com/AztecProtocol/aztec-packages/edit/next/docs/docs-developers/docs/resources/considerations/limitations.md)