...
> Source: https://docs.aztec.network/developers/docs/aztec-js/how_to_use_private_fee_juice
...
- Familiarity with [fee concepts](https://docs.aztec.network/developers/docs/foundational-topics/fees) and [Paying Fees](https://docs.aztec.network/developers/docs/aztec-js/how_to_pay_fees)
...
## How a private FPC works[​](#how-a-private-fpc-works)
...
### Two salts, not one[​](#two-salts-not-one)
...
Because neither `pay_fee` nor `mint_and_pay_fee` makes public cross-contract token calls in setup (they only deduct from the FPC's internal private balance and invoke `set_as_fee_payer`), the [setup-phase allowlist](https://docs.aztec.network/developers/docs/foundational-topics/transactions#setup-phase-non-revertible) never blocks these flows.

No refund`PrivateFPC.pay_fee()` deducts the full `max_gas_cost` and does not refund unused gas. Read `gasUsed` from a simulation (see [Estimate mana costs](https://docs.aztec.network/developers/docs/aztec-js/how_to_pay_fees#estimate-mana-costs)) to right-size your limits.

## Share one FPC address across the ecosystem[​](#share-one-fpc-address-across-the-ecosystem)
...
## Recommended salt: `0`[​](#recommended-salt-0)
...
Version-specific addressesThe `PrivateFPC` address depends on the compiled contract bytecode. A different Aztec version produces different bytecode and therefore a **different address**. Sending Fee Juice to the wrong address means **unrecoverable loss**. Before using a derived address on a given network, verify the network runs the same Aztec version as the `private-fee-juice` SDK version you have installed.

## Example: pay fees with `PrivateFPC`[​](#example-pay-fees-with-privatefpc)
...
- Review the other [fee payment methods](https://docs.aztec.network/developers/docs/aztec-js/how_to_pay_fees) available in `aztec.js`
...
- [fpc](https://docs.aztec.network/developers/tags/fpc)

[Edit this page](https://github.com/AztecProtocol/aztec-packages/edit/next/docs/docs-developers/docs/aztec-js/how_to_use_private_fee_juice.md)