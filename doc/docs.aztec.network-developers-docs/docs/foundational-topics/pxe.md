...
> Source: https://docs.aztec.network/developers/docs/foundational-topics/pxe
...
- orchestrating private function (circuit) execution and proof generation, including implementing [oracles](https://docs.aztec.network/developers/docs/aztec-nr/framework-description/advanced/protocol_oracles) needed for transaction execution
...
Privacy considerationWhen the PXE queries the node for world state (e.g., to check if a nullifier exists), the node learns which data the user is interested in. This is a known tradeoff. Users can mitigate this by running their own node.

## Components[​](#components)

### Contract Function Simulator[​](#contract-function-simulator)
...
By default, the simulator runs in a [kernelless mode](https://docs.aztec.network/developers/docs/foundational-topics/pxe/kernelless_simulations): it executes the private bytecode and computes the values the private kernels would have produced in TypeScript, instead of running the kernel circuits themselves. This is faster than a full simulation and lets the wallet capture authentication witness requests as offchain effects without prompting the user to sign during simulation.

### Proof Generation[​](#proof-generation)

After simulation, the wallet calls `proveTx` on the PXE with all of the data generated during simulation and any [authentication witnesses](https://docs.aztec.network/developers/docs/foundational-topics/advanced/authwit) (for allowing contracts to act on behalf of the user's account contract).
...
- **Notes**: Data representing users' private state. Notes are stored onchain as encrypted logs. Once discovered via [note tagging](https://docs.aztec.network/developers/docs/foundational-topics/advanced/storage/note_discovery), notes are decrypted and stored locally in the PXE.
...
- **Capsules**: Per-contract non-volatile local storage for caching computation results and persisting data across transactions. See [Using Capsules](https://docs.aztec.network/developers/docs/aztec-nr/framework-description/advanced/how_to_use_capsules) for more details.
...
Oracles are pieces of data that are injected into a smart contract function from the client side. Learn more about [how oracles work](https://docs.aztec.network/developers/docs/aztec-nr/framework-description/advanced/protocol_oracles).

## Oracle versioning[​](#oracle-versioning)
...
- [Using capsules for local storage](https://docs.aztec.network/developers/docs/aztec-nr/framework-description/advanced/how_to_use_capsules)
- [Using oracles in smart contracts](https://docs.aztec.network/developers/docs/aztec-nr/framework-description/advanced/protocol_oracles)
- [Authentication witnesses](https://docs.aztec.network/developers/docs/foundational-topics/advanced/authwit)

## Next steps[​](#next-steps)

- [Wallets](https://docs.aztec.network/developers/docs/foundational-topics/wallets) - Learn how wallets interact with the PXE
- [State management](https://docs.aztec.network/developers/docs/foundational-topics/state_management) - Understand how private state is managed
- [Note discovery](https://docs.aztec.network/developers/docs/foundational-topics/advanced/storage/note_discovery) - Learn how notes are discovered and synced

**Tags:**
- [PXE](https://docs.aztec.network/developers/tags/pxe)

[Edit this page](https://github.com/AztecProtocol/aztec-packages/edit/next/docs/docs-developers/docs/foundational-topics/pxe/index.md)