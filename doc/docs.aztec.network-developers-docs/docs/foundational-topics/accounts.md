...
> Source: https://docs.aztec.network/developers/docs/foundational-topics/accounts
...
### Why Account Abstraction Matters[​](#why-account-abstraction-matters)
...
## Aztec's Native Account Abstraction[​](#aztecs-native-account-abstraction)
...
### Breaking the DoS Attack Problem[​](#breaking-the-dos-attack-problem)
...
## How Aztec Accounts Work[​](#how-aztec-accounts-work)
...
### The Account Contract Structure[​](#the-account-contract-structure)
...
```
Address = hash(    public_keys_hash,    // All the account's public keys    partial_address      // Contract deployment information)
```
...
## The Entrypoint Pattern[​](#the-entrypoint-pattern)
...
### Non-Standard Entrypoints[​](#non-standard-entrypoints)
...
### 1. Pre-deployment (Counterfactual State)[​](#1-pre-deployment-counterfactual-state)
...
See [Creating Accounts](https://docs.aztec.network/developers/docs/aztec-js/how_to_create_account) for code examples.
...
noteAccount deployment and initialization are not required to receive notes. The user address is deterministically derived, so funds can be sent to an account that hasn't been deployed yet.

### 4. Active Use[​](#4-active-use)
...
## Authentication Witnesses (AuthWit)[​](#authentication-witnesses-authwit)
...
For detailed information about how AuthWit works in both private and public contexts, see the [Authentication Witness documentation](https://docs.aztec.network/developers/docs/foundational-topics/advanced/authwit).

## Transaction Abstractions[​](#transaction-abstractions)

Aztec abstracts two critical components of transactions that are typically rigid in other blockchains: nonces and fees.

### Nonce Abstraction[​](#nonce-abstraction)
...
### Fee Abstraction[​](#fee-abstraction)
...
This flexibility is crucial for user onboarding and enables gasless experiences. For detailed information about fee mechanics and payment options, see the [Fees documentation](https://docs.aztec.network/developers/docs/foundational-topics/fees).
...
[Edit this page](https://github.com/AztecProtocol/aztec-packages/edit/next/docs/docs-developers/docs/foundational-topics/accounts/index.md)