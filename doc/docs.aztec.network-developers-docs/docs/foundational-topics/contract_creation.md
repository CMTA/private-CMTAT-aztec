...
> Source: https://docs.aztec.network/developers/docs/foundational-topics/contract_creation
...
In the Aztec protocol, contracts are created as *instances* of contract *classes*. Unlike Ethereum where deployment is binary (deployed or not), Aztec contracts progress through multiple states before they are fully operational.
...
Not every contract needs every state. A private-only contract can skip class registration and public deployment entirely. See the [Contract Deployment Quick Reference](https://docs.aztec.network/developers/docs/aztec-nr/contract_readiness_states) for a practical guide on which steps your contract needs.
...
### Key Benefits of Contract Classes[​](#key-benefits-of-contract-classes)
...
### Structure of a Contract Class[​](#structure-of-a-contract-class)
...
### Contract Class Registration[​](#contract-class-registration)
...
### Structure of a Contract Instance[​](#structure-of-a-contract-instance)
...
### Contract Initialization vs. Public Deployment[​](#contract-initialization-vs-public-deployment)
...
## Verification of Executed Code[​](#verification-of-executed-code)
...
## Genesis Contracts[​](#genesis-contracts)
...
## Private Function Broadcasting[​](#private-function-broadcasting)
...
## Proving Contract States[​](#proving-contract-states)
...
```
use aztec::history::deployment::{    assert_contract_bytecode_was_not_published_by,    assert_contract_bytecode_was_published_by,    assert_contract_was_initialized_by,    assert_contract_was_not_initialized_by,};// Prove a contract's bytecode was published by a given blockassert_contract_bytecode_was_published_by(block_header, contract_address);// Prove a contract's bytecode was NOT published by a given blockassert_contract_bytecode_was_not_published_by(block_header, contract_address);// Prove a contract was initialized by a given block// (init_hash is the contract's initialization hash, obtainable via get_contract_instance)assert_contract_was_initialized_by(block_header, contract_address, init_hash);// Prove a contract was NOT initialized by a given blockassert_contract_was_not_initialized_by(block_header, contract_address, init_hash);
```
...
- [Contract Deployment Quick Reference](https://docs.aztec.network/developers/docs/aztec-nr/contract_readiness_states) - Practical guide for which deployment steps your contract needs
- [Deploying Contracts](https://docs.aztec.network/developers/docs/aztec-js/how_to_deploy_contract) - Deploy contracts using TypeScript
- [DApp Development Tutorial](https://docs.aztec.network/developers/docs/tutorials/js_tutorials/aztecjs-getting-started) - Build a complete application
...
[Edit this page](https://github.com/AztecProtocol/aztec-packages/edit/next/docs/docs-developers/docs/foundational-topics/contract_creation.md)