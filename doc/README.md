...
**A commitment is single-use for the recipient, not for the contract.** `PartialUintNote::complete` (aztec-nr) does not consume the commitment, and its documentation says why that matters: "the recipient only discovers the first completion, so anything carried by further ones is lost". A second `transfer_private_to_commitment` into the same commitment debits the sender again, leaves `total_supply` unchanged and credits nothing the recipient can spend — measured in `test_edge_cases.nr` (two payments of 100: the sender is down 200, the recipient holds 100). The behaviour is AIP-20's, inherited with the flow; a wallet must open a fresh commitment for every payment it expects. The contract could refuse the reuse itself — one nullifier derived from the commitment, pushed in the private half next to the completion, so a second payment is an invalid transaction and the payer's funds never move — which is open as `K-6` in [the 0.4.0 review](audits/tools/v0.4.0/CLAUDE_ANALYSIS.md) since it departs from AIP-20's behaviour; [`doc/design/commitment-reuse.md`](design/commitment-reuse.md) sets out the behaviour in both contracts and the options.
