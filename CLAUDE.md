...
- `doc/standards/aip20-features-for-cmtat.md` — which AIP-20 features the token itself could adopt while staying CMTAT-equivalent, tested against the equivalency criteria, this token's privacy premise, and measured cost. Order: note budget with recursion (measured −43,046 gates), commitment transfers screened at initialization (needs an expiry, an issuer delivery, and disclosure that the completion amount is unencrypted), a settable rule-engine hook passing recipient and caller, then the five AIP-20 renames. Holder self-burn and a single immutable minter are rejected. Also records a correction: the commitment flow *does* know the recipient at initialization.
