...
- **Private/public split per operation.** `mint_to_private`, `transfer_private_to_private`, `burn` are `#[external("private")]`: they call an inlined `#[internal("private")]` `_*_internal` that mutates notes via `self.internal`, then `self.enqueue_self` a `#[external("public")] #[only_self]` counterpart that updates `total_supply` and performs the role check plus the lifecycle check — **not-paused for `_transfer`, not-deactivated for `_mint` and `_burn`**, matching CMTAT Solidity, where a pause stops transfers but not issuance or redemption. A revert in the public part reverts the whole tx.
...
- **Batching caps.** Two globals, both **measured, not derived**. `MAX_ADDR_PER_CALL = 4` for `mint_batch` and `burn_batch` (5 breaks them). `MAX_TRANSFER_ADDR_PER_CALL = 2` for `transfer_batch` (3 and above abort with `push out of bounds`), lower because each recipient costs four constrained deliveries once the `Transfer` event is delivered constrained to both the recipient and the issuer. The nested-private-call limit is irrelevant: the `_*_internal` helpers are `#[internal]` and inlined, so a batch makes no nested calls. Raising this global means re-running the suite at the new value and updating every batch test's array literals, not re-reading the protocol constants; see *Batching limits* in `doc/README.md`.
...
└── src/modules/
    ...
    ├── validationModule.nr          # blacklist/whitelist flags, operateOnTransfer/Mint/Burn, operateOnFrom (sender-only, for the auth contracts)
...
- `doc/audits/tools/v0.3.0/CLAUDE_ANALYSIS.md` — code-quality review (not a security audit) against Aztec 5.2.0, with a measured gate-count baseline. `doc/audits/` is organised by reviewer kind and then by the release reviewed: `tools/<version>/` for tool-assisted reviews, so a report for the next release goes in a new sibling directory rather than overwriting this one. Findings carry stable IDs (`A-1`, `H-3`, …); cite them by ID in commits and code comments. Two entries are corrections of findings that measurement disproved — read those before re-proposing the change.
